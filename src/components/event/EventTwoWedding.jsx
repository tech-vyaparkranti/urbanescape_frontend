...
import { getWedding } from '../../api/getWedding';
...
import 'swiper/css/autoplay';
  ...
  function EventTwoWedding() {
      const [loading, setLoading] = useState(true);
      const [wedding, setConference] = useState(null);
      const [images, setImages] = useState([]);
  
      useEffect(() => {
          getWedding()
              .then((response) => {
                  if (response.status && response.success && response.wedding) {
                      setConference(response.wedding);
                      
                      // Parse images if they exist
                      if (response.wedding.images) {
                          try {
                              const parsedImages = JSON.parse(response.wedding.images);
                  ...
                  console.log("wedding data===========>:", response);
              })
              .catch((err) => {
                                  ...
                                  {loading ? (
                                      ...
                                      <>
                                          <h2 className="wow fadeInUp mb-20">
                                              {wedding?.title || "A Night of Hope: Our Charity Gala Room."}
                                          </h2>
                                          <div 
                                              className="font-sm wow fadeInUp"
                                              dangerouslySetInnerHTML={createMarkup(wedding?.short_desc || "")}
                                          />
                                      </>
                                  )}
                          ...
                          <div className="single__event if__is__reverse flex-column">
                              ...
                              <div className="single__event__meta">
                                  {loading ? (
                                      ...
                                      <>
                                          {/* <h2 className="wow fadeInUp mb-20">
                                              {`${wedding?.title || ""} Festival Event`}
                                          </h2> */}
                                          <div 
                                              className="font-sm wow fadeInUp"
                                              dangerouslySetInnerHTML={createMarkup(wedding?.description || "")}
                                          />
                                      </>
                                  )}
  ...
  export default EventTwoWedding;